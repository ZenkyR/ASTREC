...
export const ViewSearch = () => {
  const [valSearch, setValSearch] = useState(localStorage.getItem("search"));
  ...
  const searchArticle = () => {
    axios
      .post("https://127.0.0.1:8000/filters", {
        tri: tri,
        filters: filters,
        search: valSearch,
        prixMin: prixMin,
  ...
  useEffect(() => {
    searchArticle();
  }, [valSearch, filters, tri, prixMin, prixMax]);
      ...
      <div>
        <div className="article_sub_categ_navbar">
          ...
          <div className="article_sub_categ_tri">
            <select
              name="article_sub_categ_tri"
              id="article_sub_categ_tri"
              onClick={handleTri}
            >
